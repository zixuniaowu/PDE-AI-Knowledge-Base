---
id: hr
type: domain
title: 人事・採用
updated: 2026-09-12
---

人事は「人を評価する」業務ゆえに、AI 利用には**公平性・差別防止**という特有のハードルがあります。AI は文章量を減らす道具であり、**評価・選考の判断そのものは人間が行う**のが本プロジェクトの前提です。

## 業務の全体像

- 採用（職務定義・応募者対応・選考・案内）
- 育成（研修設計・オンボーディング）
- 労務（手続き文書・規程の整備・相談対応の一次整理）
- 人事評価の運用設計

## AI が効きそうな場面

| 業務 | AI の関わり方 | 人間に残る役割 |
|---|---|---|
| 職務定義・JD | 媒体ごとの文案のドラフト | 求める人物像の確定 |
| 面接記録 | メモの事実抽出・整理 | 評価・合否判断 |
| 研修・オンボーディング | 資料・マニュアルのドラフト | 育成方針・評価 |

## この領域特有の制約

- **差別につながる属性への言及**（年齢・性別・国籍・健康管理 etc）を AI が生成しない設計にする。応募者選別の自動化は規制・倫理の両面で慎重に
- 面接メモ・評価は要配慮の個人情報。外部 AI に入れない運用が基本
- AI が作った「それらしい評価文」が選考の実質的判断になる構造を作らない

## ユースケース

- [面接メモの事実抽出](./use-cases/interview-notes.md)

## 法規と保存期間（この領域の前提）

| 項目 | 内容 |
|---|---|
| 保存期間 | 勤怠関連書類・賃金台帳は労基法 109・110 条により **5 年**、給与所得の源泉徴収簿は 7 年。36 協定の届出も管理対象 |
| 選考情報 | 応募者情報は取得目的を明示し、不採用者の情報は一定期間後に廃棄する規程が一般的 |
| 正本システム | 人事の正本は HRMS / 勤怠 SaaS。AI は仮名化データのみを受け取り、確定登録は人事システム側で行う |

## AI 導入マップ（業務フロー × AI の掛かる場所）

```mermaid
flowchart LR
    A["採用（JD・面接・選考）"] --> B["オンボーディング"]
    B --> C["育成・評価"]
    A2["AI: JD ドラフト・<br/>メモの事実分離"] -.-> A
    B2["AI: 研修資料ドラフト"] -.-> B
    C2["AI: 評価の事実整理"] -.-> C
```
