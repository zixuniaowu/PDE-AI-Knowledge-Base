...
## As-Is 表の実フォーマット（そのまま使う）

ヒアリング結果はこの列構成で表に落とす。AI 導入の対象選定はこの表の「月次工数 × 痛み」で決める:

| 工程 | 担当（役職） | 使用システム | 帳票・入力 | 月次件数 | 所要時間/件 | 痛みの度合い |
|---|---|---|---|---|---|---|
| 入金消し込み | 経理担当 | 会計ソフト + 銀行 CSV | 入金明細 / 請求書控え | 120 件 | 6 分 | 高（差異が 15 件/月） |
| 発注書作成 | 営業事務 | 購買管理 | 発注書（様式 B） | 45 件 | 12 分 | 中 |

- 帳票は**正式名称で**書く（受注書 / 注文請書 / 納品書 / 検収書 / 請求書 / 発注書）。「伝票処理」のような概括では設計ができない

## 商慣行の数字を入れる（O2C / P2P の現実）

| 概念 | 実務の内容 |
|---|---|
| 締め日・支払サイト | 回収は「月末締め翌月末払い」等の**締めグループ**単位で動く（SAP なら支払条件 Payment Terms の設定が本質）。消し込みは締めグループごとに行う |
| 与信 | 「一次評価」の実体は与信データベース参照: 帝国データバンク / 東京商工リサーチ / リスクモンスター等。新規取引先は DB スコア + 独自基準 |
| 証憑管理 | 電帳法の検索要件（取引年月日・金額・取引先）を満たす保存構成。スキャンはタイムスタンプ運用とセット |

## KPI に基準値を入れる（名前だけでは管理できない）

| KPI | 一般的な基準値の例（業界・規模で調整） |
|---|---|
| 月次締め | 営業日 5〜10 日以内に完了 |
| 入金消し込み残件 | 月次件数の 2% 以下 |
| 発注処理リードタイム | 依頼から発注 2 営業日以内 |
| 不具合是正 | 重大 7 日以内、通常 30 日以内（担当・期限を必ず設定） |

基準値は「現状実績 → 目標 → AI 導入後の目標」の 3 列で提示すると、顧客との合意が速い。
